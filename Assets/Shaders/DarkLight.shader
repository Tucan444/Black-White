...
Shader "Unlit/DarkLight"
{
    Properties
    {
        ObjectColor ("ObjectColor", Color) = (1, 1, 1, 0)
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            fixed4 frag (v2f i) : SV_Target
            {
                ...
                a *= (min(1, dot(i.normal, position) * 0.6) + 0.6 + (1 - max(0, (_Power - m) / _Power)));
                ...
                float4 diffuseLight = float4((_Color[0] * a_) + a, (_Color[1] * a_) + a, (_Color[2] * a_) + a, 0);
                float4 color = float4(((1 - diffuseLight[0]) * ObjectColor[0]) + diffuseLight[0], ((1 - diffuseLight[1]) * ObjectColor[1]) + diffuseLight[1], ((1 - diffuseLight[2]) * ObjectColor[2]) + diffuseLight[2], 0);

                //color = float4(0, a, 0, 0);
